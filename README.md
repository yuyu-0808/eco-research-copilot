...
已完成的核心能力之外，按**可行性**分层规划长期演进方向：

### P1 · 投研增值（架构已支持，可直接迭代）

| 方向 | 定位 |
|------|------|
| **多主体横向对比** | 同时输入多家公司 / 细分赛道，按统一框架生成对比报告，逐项对标核心指标与竞争地位 |
| **轻量盈利预测** | 基于历史指标做增长率外推与场景假设，支持手动调参并自动重算（专业 DCF / 三表建模依赖外部数据源，暂不在范围内） |
| **开放 REST API** | 在现有 FastAPI 之上完善 SDK、鉴权与文档，支持嵌入企业内部投研系统与知识库 |

### P2 · 平台化演进（需引入多用户与共享存储）

| 方向 | 定位 |
|------|------|
| **团队 Workspace** | 成员共享项目、材料库、行业框架，引入用户体系与权限隔离 |
| **报告批注与版本对比** | 报告批注、checkpoint 版本对比；多人实时协作编辑成本极高，暂不承诺 |
| **专业数据接口** | 预留 Wind / Choice 等金融数据源 adapter（接口可做，数据接入需商业授权） |

### P3 · 社区生态（依赖运营与中心化服务）

| 方向 | 定位 |
|------|------|
| **行业框架贡献** | 开放自定义行业框架提交机制（现有 YAML 插件化已支持上传，进一步公开化） |
| **模板市场** | 共享研报模板、分析框架、校验规则，需中心化存储 + 审核分发机制 |
