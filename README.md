...
| 2️⃣ 检索 | **信源研究员** | 多源检索（Tavily / DuckDuckGo），给每条信源打 A-F 六级等级（借鉴 Admiralty 双维度） | 带评级的原始素材 |
...
2. 信源研究员 检索并给每条信源打 A-F 六级等级（A 官方 / B 权威媒体 / C 行业专业 / D 一般 / E 低质 / F 无法判断）→ 事实稽核官 用代码做**确定性校验**（每章证据数达标 + 核心指标 A/B/C 级支撑 + 矛盾标红），形成**检索-稽核内循环**（默认最多 3 轮）；
