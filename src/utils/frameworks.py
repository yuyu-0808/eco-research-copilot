...
与「Agent 自由拆题」相反，这里内置行业共识的研究框架：
Agent 只负责往对应章节填充证据与内容，保证输出结构 100% 符合行研规范，
框架质量不再赌模型发挥——模型越弱，这套兜底的价值越大。

设计要点：
- 每个框架按「行业概览 → 产业链 → 竞争格局 → 政策环境 → 趋势研判」五段式组织；
- 每章带「必答问题 + 核心指标 + 最少证据数 + 最低信源等级」，
  供确定性校验器（validator）在采集后逐章核对；
- build_plan 完全确定性（不依赖 LLM），架构师在此基础上才做可选微调。
"""
